...
function displayGenome(genome)
	...
	for n,cell in pairs(cells) do
		if n > Inputs or cell.value ~= 0 then
			...
			if color > 255 then color = "FFFFFF" 
			elseif color < 0 then color = "000000" end
			local opacity = "#00000040"
			if cell.value == 0 then
				opacity = "#FFFFFF50"
			end
...
item_load = iup.item {title = "Load"}
item_play = iup.item {title = "Play Top"}
...
menu = iup.menu {item_save, item_load, item_play,item_exit}

dlg = iup.dialog{text; title="Function Menu", menu=menu}
